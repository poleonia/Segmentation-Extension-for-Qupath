...
//	}
	...
	private void addMenuItem(QuPathGUI qupath) {
		var menu = qupath.getMenu("Extensions>" + EXTENSION_NAME, true);
		MenuItem menuItem = new MenuItem("Run Glo Seg");
		menuItem.setOnAction(e -> {
			try {
				// Create an instance of GLOMainCommand and pass QuPathGUI

				GLOSMainCommand glosCommand = new GLOSMainCommand(qupath);

				// Submit the task
				glosCommand.submitDetectionTask();
			} catch (Exception ex) {
				Dialogs.showErrorMessage("Error", "Failed to run GLOS Command: " + ex.getMessage());
			}
		});
		menu.getItems().add(menuItem);
	}
